...
- Specifies what you expect to send in the "TU" message in case of updated
  (corrected) call.
...
- This is a main function definition, \
  Ignoring "natural" segment boundaries, only re-send the changed part,
  from the first through the last changed characters or numbers. \
  This perhaps is a somewhat rare (avantgarde) function... \
  The only modifiers to this function are those independent of segments.
...
- This is a main function definition. \
  ...
  This is a rather strict function,
  meaning that the full updated segment(s), including eventual leading
  and trailing __"/"__ characters will be sent in the final message. \
...
- This is a main function definition. \
  ...
  However, still __W/__ will be repeated e.g. for __REQ_GT1_CHARS_SENT__. \
...
- This is a main function definition. \
  ...
  However, still __W/__ or __W1__ will be sent e.g. for
...
- This is a main function definition, using an intuitive (friendly, shorthand,
  ...
  \
  __SEND_SHORTEST,REQ_GT1_CHARS_SENT__
...
- This is a main function definition, using an intuitive (friendly, shorthand,
  alias) name for the following parameter combination (note that these options
  implicitly also imply __FULL_3CHAR_CALLS__: \
  \
  __SEND_FULL_SEGMENTS,REQ_GT1_CHARS_SENT,REQ_GT1_CHARS_SPARED,
  REQ_MATCHED_CHAR__
...
- This is a main function definition, using an intuitive (friendly, shorthand,
  ...
  implicitly also imply __FULL_3CHAR_CALLS__: \
  \
  __SEND_BASE_SEGMENTS,REQ_GT1_CHARS_SENT,REQ_GT1_CHARS_SPARED,
  REQ_MATCHED_CHAR__
...
- This is a main function definition, using an intuitive (friendly, shorthand,
  ...
  implicitly also imply __FULL_3CHAR_CALLS__: \
  \
  __SEND_TRUNCATED_SEGMENTS,REQ_GT1_CHARS_SENT,REQ_MATCHED_CHAR__
...
- This is a main function definition, using an intuitive (friendly, shorthand,
  alias) name for the following parameter combination: \
  \
  __SEND_TRUNCATED_SEGMENTS,REQ_GT1_CHARS_SENT__
...
__FULL_3CHAR_CALLS__

- This is an optional modifier. This means that the updated (corrected) full
  call must be re-sent in the "TU" message when the full call contains only
  three characters.
...
- This is an optional, however very strict modifier. This means to always
  re-send full call in the "TU" message when more than one characters updated.
...
- This is an optional, strict while somewhat milder modifier than
  __FULL_FOR_GT1_CHAR_UPD__. This means to always re-send full call in the
  "TU" message when more than one segment updated. This is meant to help when
  first you only received a partial call while listening to many, e.g. first
  only copying __HA5C__, then it would be enough to resend __CQZ__ instead of full
  __HA5CQZ__, although updated two characters.
...
- This is an optional, strict however rarely hit modifier. This means
  to always re-send full call in the "TU" message when a slash __"/"__
  present, in order  to spare ambigousity in segments, since no simple
  and bullet-proof prefix/suffix logic can be implemented if slash present.
...
- This is an optional, however probably one of the most useful modifiers.
  ...
  the correction with a single __"R"__. Instead, upon other modifiers,
  at least __6R__ or __G6R__ should be sent in the "TU" message.

- For __SEND_SHORTEST__, this means that when only updating a single
...
- For __SEND_*_SEGMENTS__ functions, in case when the truncated remaining
...
- This is an optional modifier, meaning that the final returned part should
  ...
  be resent, using the same expanding logic as for __REQ_GT1_CHARS_SENT__. \
...
- This is an optional modifier. This means that do not use the shortest
  possible truncation if only sparing a single character. \
  That is, for __SEND_TRUNCATED_SEGMENTS__ with this option, when correcting
  __W1XYA__ to __W2XYZ__, truncated __2XYZ__ is not enough because only
...
- This is an optional modifier. In order to satisfy other options, like
  __REQ_GT1_CHARS_SENT__ or __REQ_MATCHED_CHAR__, it may be required to add
  ...
  for both __RELAXED__ and __AVANTGARDE__. However, using this option,
  __K2__ will be preferred since it needs less CW keying time to send. \
  \
  Another example, for __AVANTGARDE__, when correcting __HA5SI/P__ to
  ...
  Note that in this example, for __RELAXED__, __SE__ will always be selected
...
For examples, please try the demo script CONFIG_TU_RESENDUPDATEDCALL_demo.sh
